...
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    const params = {
      ...
      spring: 0.4,
      friction: 0.5,
      startWidth: 5,  // Reduced start width
      endWidth: 1,    // Reduced end width
      cursorSize: 25,
      yOffset: 10,     // Reduced vertical offset
      fadeOutDuration: 500  // Duration for fade out effect in ms
    };
    ...
    const update = () => {
      ...
      trail.forEach((p, pIdx) => {
        ...
        // Apply vertical offset to the first point
        if (pIdx === 0) {
      ...
      for (let i = 0; i < trail.length - 1; i++) {
        ...
        const alpha = opacity * (1 - t * 0.5);  // Tail fades out towards the end

        ctx.beginPath();
        ctx.moveTo(trail[i].x, trail[i].y);
        ctx.lineTo(trail[i + 1].x, trail[i + 1].y);
        ctx.lineWidth = width;
        ctx.strokeStyle = `rgba(222, 41, 16, ${alpha})`;  // Chinese red with dynamic opacity
        ctx.lineCap = "round";
        ctx.lineJoin = "round";
        ctx.stroke();
      }
